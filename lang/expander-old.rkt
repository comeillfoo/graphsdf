...
(define-macro (gsdf-module-begin BODY ...)
  #'(#%module-begin (gsdf-program BODY ...)))
...
(define-macro-cases expand-op
  [(expand-op + A B)]
  [(expand-op - A B)]
  [(expand-op / A B)]
  [(expand-op * A B)]
  [(expand-op sqrt A)]
  [(expand-op define ID VALUE)])
  ; #'(lambda (nodes queues next-node-id next-queue-id)
  ;   (let
  ;     ([f
  ;       (match (list (identifier? A) (identifier? B))
  ;         [(list #f #f) (compose B A)]
  ;         [(list #f #t) A]
  ;         [(list #t #f) B]
  ;         [else +])])
  ;     (set!-values
  ;       (nodes queues next-node-id next-queue-id)
  ;       ((compose A B) nodes queues next-node-id next-queue-id))
  ;     (values nodes queues next-node-id next-queue-id))))
    (values nodes queues next-node-id next-queue-id)

(define-macro (define ID VALUE)
  #'(expand-op define ID VALUE))
(provide define)
